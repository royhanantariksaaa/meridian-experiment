...
const DEFAULT_DETAIL_DELAY_MS = 80;
const DEFAULT_CACHE_TTL_MS = 60_000;
const DEFAULT_FETCH_TIMEOUT_MS = Number(process.env.MARKET_FETCH_TIMEOUT_MS || 8_000);
...
function safeKey(value) {
  return String(value || "default").replace(/[^a-zA-Z0-9_.-]+/g, "_").slice(0, 140);
}
...
async function fetchWithTimeout(url, options = {}, timeoutMs = DEFAULT_FETCH_TIMEOUT_MS) {
  ...
  try {
    ...
    if (error?.name === "AbortError") throw new Error(`market fetch timeout after ${timeoutMs}ms: ${url}`);
    throw error;
  } finally {
    clearTimeout(timer);
  }
...
function normalizeOfficialPool(pool, { metricWindow }) {
  ...
  return {
    ...
    pool_type: "dlmm",
    bin_step: numeric(pool?.pool_config?.bin_step),
    fee_pct: numeric(pool?.pool_config?.base_fee_pct ?? pool?.dynamic_fee_pct),
...
function applyFirstPassFilter(pool) {
  ...
  if (pool.dev && isDevBlocked(pool.dev)) return "local dev blocklist";
...
function applyFinalFilter(pool) {
  const s = config.screening;
...
async function fetchOfficialPage({ page, pageSize, sortBy, ttlMs }) {
  const cacheKey = `official-fast-pools-${page}-${pageSize}-${sortBy}`;
...
async function fetchPoolDiscoveryDetail({ poolAddress, timeframe, ttlMs }) {
  const cacheKey = `official-fast-detail-${timeframe}-${poolAddress}`;
  const cached = readCache(cacheKey, ttlMs);
  ...
  const res = await fetchWithTimeout(`${POOL_DISCOVERY_BASE}/pools?${params.toString()}`);
...
async function enrichDetails(pools, { timeframe, limit, delayMs, ttlMs }) {
  const enriched = [];
  const errors = [];
...
export async function scanOfficialFastCandidates({
  limit = 10,
  ...
  const metricWindow = String(timeframe || "1h").trim() || "1h";
  ...
  for (const pool of normalized) {
    const reason = applyFirstPassFilter(pool);
  ...
  const { enriched, errors: detailErrors } = await enrichDetails(rankedFirstPass, {
    timeframe: metricWindow,
    limit: Math.max(limit, detailLimit),
  ...
  for (const pool of enriched) {
    const reason = applyFinalFilter(pool);
